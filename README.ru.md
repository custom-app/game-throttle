...
Библиотека ничего не перехватывает сама — обёртку надо поставить туда, откуда игра ходит на сервер.
...
#### Когда обёртку некуда вставить

Бывает, что запросы уходят из чужого кода: SDK платёжки, аналитика, загрузчик ассетов внутри
движка. Дотянуться до их `fetch` нельзя. На этот случай есть установщики — они один раз подменяют
глобальные `fetch` и `EventSource`:
